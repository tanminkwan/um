아래는 **“Proxy 서버가 MIP SDK로 라벨 → 암호화 → 청크 업로드 → 청크 다운로드 → SDK 복호화 → 복호화 파일 전달”** 흐름을 *한 줄도 생략 없이* 담은 **완결 교재 v4.1 (2025-06-20)** 입니다.

> 모든 표·코드·다이어그램을 그대로 복사해 Wiki / PDF / 슬라이드로 배포하거나 실습 환경(Win Server 2022 + PS 7.4 + .NET 6)에서 바로 실행할 수 있습니다.
